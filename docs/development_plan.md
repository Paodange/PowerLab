...
1. 开始前确认主分支工作区干净，并从最新主分支创建任务分支；
2. 所有实现和验证只在任务分支进行；
3. 验证通过后在任务分支创建一个内容清晰的提交；
4. 切回主分支，以非快进方式合并任务分支；
5. 合并后重新执行受影响的构建和测试；
6. 遇到已有未提交修改、合并冲突或分支被其他任务占用时立即停止并报告，不得 reset、stash、强制切换或覆盖他人修改。

任务分支统一命名为 `task/tNN-short-name`。除非用户明确要求，不推送远端、不删除任务分支。
